    ...
    return(
        <div className="card bg-neutral-800/50 w-full sm:w-80 shadow-sm">
            ...
            <div className="card-body p-4 sm:p-6">
                <h2 className="card-title mb-4 sm:mb-6 text-base sm:text-lg">{props.note.m_title}</h2>
                <p className="text-xs bg-teal-950 p-2 rounded-xl">{props.note.m_description}</p>
                <div className="*:m-1 bg-black/10 flex flex-wrap gap-1 rounded-2xl p-2">
                    <div className="w-fit *:ml-1">
                        <div className="text-sm sm:text-base">{props.note.t_name}</div>
                        <div className=" w-fit text-xs text-white/20">{props.note.t_designation}</div>
                    </div>
                </div>
                <div className="*:m-1 bg-black/10 flex flex-wrap gap-1 rounded-2xl p-2">
                    <p className="text-xs my-auto">Contributed by</p>
                    <div className="badge w-fit flex-wrap sm:flex-nowrap">
                        <span className="text-xs sm:text-sm">{props.note.name}</span>
                        <div className="w-fit p-1 text-xs text-white/10">{props.note.dept}</div>    
                        <div className="w-fit p-1 text-xs text-white/10">{props.note.batch}</div>
                    </div>
                </div>
                <div className="card-actions justify-end mt-2">
                    {props.note.file_location && (
                        <a
                        href={props.note.file_location}
                        target="_blank"
                    ...
                    onClick={() => {
                        ...
                        link.href = props.note.file_location;
                        link.download = props.note.m_title; // optional: default file name
                        link.click();
                    }}
                    >
