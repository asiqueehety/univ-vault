...
export default function PracticeCard(props : PracticeCardProps)
{
    ...
    return(
        <div className="card bg-neutral-800/50 shadow-sm w-full sm:w-fit flex flex-col sm:flex-row">
            {/*I wanna show a preview of the document in here. The preview will be its first page*/}
            <div className="card-body w-full sm:w-80 p-4 sm:p-6">
                <h2 className="card-title mb-4 sm:mb-6 text-base sm:text-lg">{props.prac.q_title}</h2>
                <div className="*:m-1 bg-black/10 flex flex-wrap gap-1 rounded-2xl p-2">
                    <div className="w-fit *:ml-1 flex flex-col sm:flex-row">
                        <div className="text-sm sm:text-base">{props.prac.q_type=="term"? <>Semester Final</> : props.prac.q_type=="ct"? <>Class Test</>:<>Books / others ...</>}</div>
                        <div className="w-fit p-1 text-xs text-white/10">taken in {props.prac.year}</div>
                    </div>
                </div> 
                {props.prac.t_name &&
                <div className="*:m-1 bg-black/10 flex flex-wrap gap-1 rounded-2xl p-2">
                    <div className="w-fit *:ml-1">
                        <div className="text-sm sm:text-base">{props.prac.t_name}</div>
                ...
                <div className="*:m-1 bg-black/10 flex flex-wrap gap-1 rounded-2xl p-2">
                    ...
                    <div className="badge w-fit flex-wrap sm:flex-nowrap">
                        <span className="text-xs sm:text-sm">{props.prac.name}</span>
                        <div className="w-fit p-1 text-xs text-white/10">{props.prac.dept}</div>    
                        <div className="w-fit p-1 text-xs text-white/10">{props.prac.batch}</div>
                    </div>
                </div>
            ...
            <AnimatePresence>
                {
                    ...
                    className="m-2 max-w-full overflow-hidden"
                    >

                        <Image alt={`preview of ${props.prac.q_title}`} src={props.prac.file_location} width={800} height={200} className="w-full h-auto max-w-[800px]" onClick={()=>setShowPreview(false)}/>
                    </motion.div>
                }    
            </AnimatePresence>
